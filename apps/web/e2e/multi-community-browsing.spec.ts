...
import { ALPHA_ID, BETA_ID, PROPOSAL_ID, installPublicFixtures } from "./fixtures";

const ALPHA_NAME = "Alpha Builders";
const BETA_NAME = "Beta Citizens";
const ALPHA_PROPOSAL = "Fund Alpha treasury observability";
...
test.describe("multi-community public browsing", () => {
  test("browses registry, scoped proposal history, and canonical detail routes", async ({ page }) => {
    ...
    await page.getByRole("link", { name: `View ${ALPHA_NAME} community details` }).click();
    ...
    await page.getByRole("link", { name: new RegExp(`View proposal ${PROPOSAL_ID}`) }).click();
    await expect(page).toHaveURL(`/communities/${ALPHA_ID}/proposals/${PROPOSAL_ID}`);
    await expect(page.getByRole("heading", { name: ALPHA_PROPOSAL })).toBeVisible();
    const breadcrumb = page.getByRole("navigation", { name: "Breadcrumb" });
    await expect(breadcrumb).toContainText(ALPHA_NAME);
    await expect(breadcrumb).toContainText("Proposals");
  });

  test("community search query is URL-backed and survives navigation", async ({ page }) => {
    await page.goto("/communities");
    ...
    await page.getByRole("link", { name: `View ${BETA_NAME} community details` }).click();
    await expect(page).toHaveURL(`/communities/${BETA_ID}`);
    await page.goBack();
    await expect(searchInput).toHaveValue(BETA_NAME);
  });

  test("switching communities never leaks proposals with a shared id", async ({ page }) => {
    ...
    await page.getByRole("link", { name: new RegExp(`View proposal ${PROPOSAL_ID}`) }).click();
    await expect(page).toHaveURL(`/communities/${BETA_ID}/proposals/${PROPOSAL_ID}`);
    await expect(page.getByRole("navigation", { name: "Breadcrumb" })).toContainText(BETA_NAME);
  });

  test("breadcrumb context remains reachable at mobile and desktop widths", async ({ page }) => {
