...
| `NEXT_PUBLIC_COMMUNITY_FACTORY_CONTRACT_ID` | Testnet CommunityFactory fixture |
...
| `npm run test --workspace=web` | Run frontend unit tests |
...
2. **Community creator** reviews metadata and governance parameters
3. **Community creator** deploys through the configured CommunityFactory
4. **Community owner** mints NFT to a member with an IPFS metadata URI
5. **Member** delegates voting power to themselves
6. **Proposer** creates a proposal (needs delegated NFT voting power)
7. **Member** casts a vote on the proposal detail page
